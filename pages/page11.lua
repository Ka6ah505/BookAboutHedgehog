...
local function onPageSwap( event )
    ...
    elseif event.phase == "ended" or event.phase == "cancelled" then
    	...
    	if distance > swipeThreshNext then
	    	storyboard.removeScene( "pages.page11" )
			storyboard.gotoScene( "pages.page12", "slideDown", 400  )
		elseif distance < swipeThreshPrev and distance < 0 then
			storyboard.removeScene( "pages.page11" )
			storyboard.gotoScene( "pages.page10", "slideUp", 400  )
	    end
...
function scene:createScene( event )
	...
	background = display.newImageRect( "images/12.jpg", display.contentWidth, display.contentHeight)
