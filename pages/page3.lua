...
local function onPageSwap( event )
    ...
    elseif event.phase == "ended" or event.phase == "cancelled" then
    	...
    	if distance > swipeThreshNext then
	    	storyboard.removeScene( "pages.page3" )
			storyboard.gotoScene( "pages.page4", "zoomInOutRotate", 400  )
		elseif distance < swipeThreshPrev and distance < 0 then
			storyboard.removeScene( "pages.page3" )
			storyboard.gotoScene( "pages.page2", "zoomOutInRotate", 400  )
	    end
...
function scene:createScene( event )
	...
	background = display.newImageRect( "images/4.jpg", display.contentWidth, display.contentHeight)
